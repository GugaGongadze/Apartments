...
export default function injectMiddleware(app: Application, server: Express) {
  ...
  server.use('/apartments', (req: RequestWithUser, res, next) => {
    if (!req.user) {
      const error = new ErrorHandler(ErrorCode.Unauthorized, 'Unauthorized')
      return next(error)
    }
    ...
    if (
      req.method === 'POST' &&
      req.user.permission === UserPermission.Client
    ) {
      const error = new ErrorHandler(ErrorCode.Forbidden, 'Action Forbidden')
      return next(error)
    }
  ...
  server.post('/apartments', (req: RequestWithUser, res, next) => {
    if (!req.user) {
      const error = new ErrorHandler(ErrorCode.Unauthorized, 'Unauthorized')
      return next(error)
    }

    if (req.user.permission === UserPermission.Client) {
      const error = new ErrorHandler(ErrorCode.Forbidden, 'Action Forbidden')
      return next(error)
    }
  ...
  server.use('/apartments/:apartmentId', (req: RequestWithUser, res, next) => {
    if (!req.user) {
      const error = new ErrorHandler(ErrorCode.Unauthorized, 'Unauthorized')
      return next(error)
    }
